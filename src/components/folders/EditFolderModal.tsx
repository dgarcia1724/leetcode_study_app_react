...
interface EditFolderModalProps {
  isOpen: boolean;
  onClose: () => void;
  onEditFolder: (folderId: number, folderName: string) => Promise<void>;
...
const EditFolderModal: React.FC<EditFolderModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (currentFolder) {
        await onEditFolder(currentFolder.id, folderName.trim());
        onClose();
      }
    } catch (err) {
      setError("Failed to edit folder. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
